...
export async function POST(request: NextRequest) {
  ...
  const rule = body.ruleId
    ? await prisma.crisisRule.update({ where: { id: body.ruleId }, data: { status: "ACTIVE", startsAt: { set: new Date() } } })
  ...
  if (!body.ruleId) await prisma.crisisRule.update({ where: { id: rule.id }, data: { status: "ACTIVE", startsAt: { set: new Date() } } });
