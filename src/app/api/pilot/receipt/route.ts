...
export async function POST(request: NextRequest) {
  ...
  // Only the ledger write decides whether the receipt succeeded. Allocation ran
  // inside this try before, so a failure there reported "could not save" for a
  // delivery that had already committed — and the operator, believing nothing
  // happened, re-entered the same tanker and doubled the stock.
  try {
  ...
  // The delivery is recorded. Allocation is a separate step, and if it fails the
  // receipt still stands — the operator is told allocation needs re-running
  // rather than being invited to enter the delivery twice.
  try {
    ...
    return NextResponse.json({
      success: true,
      allocation: null,
      warning: "حُفظت الكمية، لكن تعذر تشغيل التخصيص التلقائي. شغّله من لوحة الإدارة."
    });
  }
}
